...
typedef void(^JOConnectSuccess)(JOBluetoothDevice *device);
typedef void(^JOConnectFail)(JOBluetoothDevice *device);
typedef void(^JODisconnect)(JOBluetoothDevice *device);
...
@property (nonatomic, copy) JOConnectSuccess onConnectSuccess;
@property (nonatomic, copy) JOConnectFail onDisconnect;
@property (nonatomic, copy) JODisconnect onConnectFailure;
 ...
 *
 */
-(id) init:(id<JOBluetoothRadioDelegate>)delegate centralManager:(CBCentralManager *)centralManager;
...
/*!
 *  @method CencelDiscovery:
 *
 *  @param none
 *
 *  @discussion 取消查询周围的蓝牙设备
 *
 */
